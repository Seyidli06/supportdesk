...
class ChangeTicketStatusApplicationServiceTest {
    ...
    void setUp() {
        ...
        existingTicket = new Ticket(ticketId, "Database Down", "Production DB error", Instant.now());
    }

    @Test
    @DisplayName("Agent should successfully change ticket status to IN_PROGRESS")
    void agentCanChangeStatus() {
        UserContext agentContext = new UserContext("agent-1", UserRole.AGENT);
        ChangeTicketStatusCommand command = new ChangeTicketStatusCommand(ticketId.getValue().toString(), TicketStatus.IN_PROGRESS);

        when(ticketRepository.findById(ticketId)).thenReturn(Optional.of(existingTicket));
        when(ticketRepository.save(any(Ticket.class))).thenAnswer(invocation -> invocation.getArgument(0));

        TicketResult result = service.changeStatus(command, agentContext);
    ...
    @Test
    @DisplayName("Regular user should be denied status change access")
    void regularUserDenied() {
        UserContext userContext = new UserContext("user-1", UserRole.USER);
        ChangeTicketStatusCommand command = new ChangeTicketStatusCommand(ticketId.getValue().toString(), TicketStatus.IN_PROGRESS);

        assertThrows(UnauthorizedAccessException.class, () -> service.changeStatus(command, userContext));
